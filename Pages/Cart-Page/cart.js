...
async function displayProduct() {
  const cartArr = await cartFetch("https://tangle-teezer-db.onrender.com/cart");
...
document.addEventListener("click", async (e) => {
  if (e.target.classList.contains("removebtn")) {
    ...
    try {
      await fetch(`https://tangle-teezer-db.onrender.com/cart/${id}`, {
        method: "DELETE",
      });
...
cartContainer.addEventListener("click", async (e) => {
  if (
    ...
    try {
      const res = await fetch(`https://tangle-teezer-db.onrender.com/cart/${id}`);
...
async function sendingData() {
  const res = await cartFetch("https://tangle-teezer-db.onrender.com/cart");
}
sendingData();